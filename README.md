...
- 🎞️ **内置转换器**（纯 Node、零依赖、**无需任何外部工具**）：解析 PKG/TEX（含 LZ4/DXT 解码、GIF89a/LZW 编码）——把**视频纹理**提取为 mp4、把**动画序列纹理**转成动画 GIF，存入专用文件夹；**扫描后自动转换**符合条件的场景壁纸（无需手动逐个点转换）；**转换带实时进度条**（作业轮询）；**一键打开文件夹**管理/删除；点选即设为背景
...
| **WE 壁纸** | 扫描按钮（**扫描即自动转换**符合条件的场景壁纸）/ 壁纸库路径（自动探测失败时手动填）/ 壁纸缩略图网格（场景卡片标记「可转换」或「无可用纹理」，后者不显示转换按钮） |
...
| scene（含视频纹理 / 动画序列） | ✅ 动态 | 扫描后**自动转换**（提取 mp4 / 动画 GIF）；卡片标记「可转换」 |
| scene（纯 3D / 粒子） | ⚠️ 静态 | 无可用纹理，不显示转换按钮；点击先用预览图，后台尝试其公开预览视频（需联网），失败保留预览图 |
