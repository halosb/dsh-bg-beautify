...
- 💫 **输入框呼吸光晕**（v0.2.1）：输入框外圈**均匀弥散软光匀速呼吸**，光晕背后一层**超大模糊的旋转色晕**——两个颜色按 abab… 环绕分布、柔和流转（无任何灯带/环/线条），并随光晕一起变色；**颜色 / 速度 / 交叉色 / 强度(0~2)**可调；尊重系统"减少动态效果"
...
| 设置 | 说明 |
|---|---|
| 输入框呼吸光晕 | 开关 + 光晕颜色 + 光晕速度（0.3s~5s）+ 使用交叉色（开关与第二颜色）+ 光晕强度（0~2）；均匀弥散软光匀速呼吸 + 背后旋转色晕（两色 abab 环绕、柔和流转，无灯带/线条） |
| 背景图 | 文本框：`/bg/文件名` / `https://...` / `data:image/...`；留空 = 无图 |
| 从本地选择图片 | 上传到插件 `assets/`，自动填入 `/bg/文件名` |
| 四个透明度滑块 | 主区 / 侧边栏 / 卡片浮层 / 输入区，0（全透）～ 1（不透），越小背景图越明显 |
| 文字颜色 | 白色 / 黑色 / 跟随主题；浅色模式下建议白色（默认）。白字只作用于**透明主区**（会话区/欢迎页/欢迎语标题），自带底色的子区（消息气泡、输入条、提问/审批卡片、侧边栏）保持黑字可读，不会白字压白底 |
| 背景纱幕 | 在背景图上叠加一层半透明纱幕，整体对比更强（默认关闭） |
| 浏览器图标 | 换 favicon：`/bg/xxx.svg`、外链或 data URI |
| 欢迎语 | 替换空会话页欢迎语 |
| 浏览器标题后缀 | 追加到标签页标题末尾 |
| 尺寸 / 位置 / 背景固定 | cover / contain / auto；CSS position；大图建议取消固定（Chrome 对固定大背景会模糊） |
| 恢复默认 | **只重置四个透明度**到出厂值，背景图与显示方式不变 |

> 启动页（加载中）文案由外壳渲染，插件无法修改。
...
| `brandIcon` / `welcomeText` / `titleSuffix` | '' / '' / '' | 品牌定制：favicon、欢迎语、标题后缀（留空 = 默认） |
| `glowEnabled` | true | 输入框呼吸光晕开关（默认开启） |
| `glowColor` | `#e8a8d0` | 光晕主色（`#rrggbb`） |
| `glowSpeed` | 2 | 光晕速度：一个呼吸周期秒数（0.3～10） |
| `glowCross` | true | 使用交叉色：主色 ↔ 交叉色交替 |
| `glowCrossColor` | `#9ec5ff` | 交叉色（`#rrggbb`） |
| `glowStrength` | 0.45 | 光晕强度（0～1.5，上限经过调校，最强时仍可控） |
...
| 光晕选项不见了 | v0.2.1 起为"输入框光晕"分组；请重启 `dsh web`（客户端 bundle 与 host 端字段校验都在重启时加载） |
