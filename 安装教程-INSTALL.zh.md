...
2. 应看到：默认无背景图（只做半透明），主区/侧边栏/卡片呈半透明；在 背景美化 → 背景图 里选图后整页出现背景。
...
| 文字颜色 | 白色 / 黑色 / 跟随主题；白字只作用于**透明主区**（会话区/欢迎页/欢迎语标题），自带底色的子区（消息气泡、输入条、提问/审批卡片、侧边栏）自动保持黑字可读 |
...
| 区域 | 默认 |
|---|---|
| 背景图 | `''`（默认无图，只做半透明；可在设置里选图） |
| 主区 / 聊天区 | 0.25 |
| 侧边栏 | 0.5 |
| 卡片 / 浮层 | 0.7 |
| 输入区 | 0.75 |
| 背景纱幕 | 开启 |
| 输入框呼吸光晕 | 开启；颜色 `#e8a8d0`、速度 2s、交叉色开启 `#9ec5ff`、强度 0.45 |
...
| 设置页没有"输入框光晕"分组 | v0.2.1 起为"输入框光晕"分组；需重启 `dsh web`（host 端字段校验与客户端 bundle 都在重启时加载）；装的是旧版则先更新插件 |
